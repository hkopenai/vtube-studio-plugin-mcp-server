...
export const getParameterValue = {
    ...
    description: 'Retrieves the value of a specific parameter (default or custom) from VTube Studio',
    inputSchema: {
        name: z.string().describe('Name of the parameter to retrieve the value for.')
    },
    ...
    execute: async (ws: WebSocket | null, args: { name: string }) => {
        return new Promise((resolve, reject) => {
            ...
            const requestId = 'parameter-value-request-1';
            ...
            const messageHandler = (event: any) => {
                ...
                if (message.requestID === requestId && message.messageType === 'ParameterValueResponse') {
            ...
            setTimeout(() => {
                ws.removeEventListener('message', messageHandler);
                reject(new Error('Timeout waiting for parameter value response.'));
            }, 5000);
