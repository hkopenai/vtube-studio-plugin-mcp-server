...
export const checkFaceFound = {
    ...
    execute: async (ws: WebSocket | null, args: Record<string, never>) => {
        return new Promise((resolve, reject) => {
            ...
            const requestId = 'face-found-request-1';
            ...
            const messageHandler = (event: any) => {
                ...
                if (message.requestID === requestId && message.messageType === 'FaceFoundResponse') {
            ...
            setTimeout(() => {
                ws.removeEventListener('message', messageHandler);
                reject(new Error('Timeout waiting for Face Found response.'));
            }, 5000);
