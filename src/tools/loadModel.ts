...
export const loadModel = {
    name: 'loadModel',
    title: 'Load VTS Model',
    description: 'Loads a VTube Studio model by its ID',
    inputSchema: {
        modelID: z.string().describe('ID of the model to load. Provide an empty string to unload the current model.')
    },
    ...
    execute: async (ws: WebSocket | null, args: { modelID: string }) => {
        return new Promise((resolve, reject) => {
            ...
            const requestId = 'model-load-request-1';
            ...
            const messageHandler = (event: any) => {
                ...
                if (message.requestID === requestId && message.messageType === 'ModelLoadResponse') {
            ...
            setTimeout(() => {
                ws.removeEventListener('message', messageHandler);
                reject(new Error('Timeout waiting for model load response.'));
            }, 5000);
